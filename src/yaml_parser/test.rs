...
fn construct_from_string_and_match_type(stringType: &str, expect: ParameterType) -> Result<(), String> {
...
fn types_match_or_error(expect: ParameterType, actual: Result<ParameterType, String>) -> Result<(), String> {
    match actual {
...
fn construct_and_match_type_from_array(input_iterator: Box<dyn Iterator<Item=&str>>, expect: ParameterType) ->
Result<(), String> {
...
";
    let parsed : CodesFile = serde_yaml::from_str(test_input).unwrap();
        ...
        assert_eq!(ParameterType::String, parsed.codes[&0].instruction.as_ref().unwrap().parameters[0].data_type);

        assert_eq!("a_bool", parsed.codes[&0].instruction.as_ref().unwrap().parameters[1].name);
        assert_eq!(ParameterType::Bool, parsed.codes[&0].instruction.as_ref().unwrap().parameters[1].data_type);
        assert_eq!("a_byte", parsed.codes[&0].instruction.as_ref().unwrap().parameters[2].name);
        assert_eq!(ParameterType::Uint8, parsed.codes[&0].instruction.as_ref().unwrap().parameters[2].data_type);
        Ok(())
    }
