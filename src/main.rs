...
fn parse_arguments(argv: Vec<String>) -> Result<Arguments, ()> {
    ...
    let matches = match options.parse(&argv[1..]) {
        Ok(m) => { m }
        Err(f) => { 
            println!("{}", options.usage(format!("Usage: {} -i INPUT [-r RUST_TARGET] [-c C_SOURCE] [-c C_HEADER]", argv[0]).as_str()));
            return Err(());
        }
    };
...
fn parse_input_file_and_generate_outputs(input_file: File, opts: Arguments) -> Result<(), serde_yaml::Error> {
...
fn main() -> Result<(), serde_yaml::Error> {
    ...
    let opts = parse_arguments(commandline_arguments)
        .unwrap_or_else(|e| {std::process::exit(0)});
