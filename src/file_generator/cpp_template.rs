...
impl FileGenerator for CppHeaderGenerator {
    ...
    fn write_enumerations(
        ...
        self.file.write_all(
            self.get_formatted_enumeration_codes(&parameters, "INST", |c| c.instruction.is_some()).as_bytes()
        )?;
        self.write_enumerations_footer("Instructions")?;
        
        self.write_enumerations_header("Feedbacks")?;
        self.file.write_all(
            self.get_formatted_enumeration_codes(&parameters, "FB", |c| c.feedback.is_some()).as_bytes()
        )?;
...
"#,
                        ...
                        inst.parameters
                            .iter()
                            .map(|p| {
                                p.c_parameter_definition_with_comment()
                            })
                            .collect::<Vec<String>>()
                            .join("\n\t")
                    )
...
"#,
            Path::new(&header_file.clone().unwrap_or(source_file_name.clone()))
                .file_name()
                .unwrap()
                .to_str()
                .unwrap()
