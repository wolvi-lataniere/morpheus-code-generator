...
pub enum Instructions {
    ...
    file.write_all(parsed_codes.iter().map(|(_, inst, _)| inst.clone()).collect::<Vec<String>>().join(",\n\t").as_bytes())?;
...
pub enum Feedbacks {
    ...
    file.write_all(parsed_codes.iter().map(|(_, _, fb)| fb.clone()).collect::<Vec<String>>().join(",\n\t").as_bytes())?;
